...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div className="modal">
        <div onClick={()=>setModalState(false)} className={`modalOverlay ${modalStatus ? 'modalShow' : ''  } `} />
        <div className={`modalDiv ${modalStatus ? 'modalDivShow' : '' }  `}>
          <h1 className="text-3xl font-bold m-5 ENQUIREHEADING">ENQUIRE NOW <span onClick={()=>setModalState(false)}>&times;</span></h1>
        </div>
      </div>
